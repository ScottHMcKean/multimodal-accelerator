...
retriever_tool = create_retriever_tool(
  vs_retriever,
  "document_retriever",
  "use this to look up documents that may be relevant to the query", 
)
...
try:
    ...
    agent_with_raw_output = create_tool_calling_agent(
        llm, tools, "Use the retriever tool to answer the following questions"
    )
except KeyError:
    agent_with_raw_output = create_tool_calling_agent(llm, tools)
agent = agent_with_raw_output | RunnableGenerator(wrap_output) | ChatCompletionOutputParser()
...
for event in agent.stream({"messages": [{"role": "user", "content": "What dams have failed?"}]}):
    print(event, "---" * 20 + "\n")
